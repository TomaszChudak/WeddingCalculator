...
describe.each([2020, 2021, 2022])("Price should be lower in we don't select base service", (year: ServiceYear) => {
    test("Don't count BlurayPackage without VideoRecording", () => {
        const result = calculatePrice(["BlurayPackage"], year);
        expect(result).toEqual({ basePrice: 0, finalPrice: 0 });
    });

    test("Don't count TwoDayEvent without VideoRecording or Photography", () => {
        const result = calculatePrice(["TwoDayEvent"], year);
        expect(result).toEqual({ basePrice: 0, finalPrice: 0 });
    });
});
